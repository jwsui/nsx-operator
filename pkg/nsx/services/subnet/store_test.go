...
func Test_IndexFunc(t *testing.T) {
	id, tag, scope := "test_id", "test_tag", "nsx-op/subnet_cr_uid"
	subnet := model.VpcSubnet{
		Id:   &id,
		Tags: []model.Tag{{Tag: &tag, Scope: &scope}},
	}
	t.Run("1", func(t *testing.T) {
		got, _ := indexFunc(subnet)
...
func Test_InitializeSubnetStore(t *testing.T) {
	...
	subnetCacheIndexer := cache.NewIndexer(keyFunc, cache.Indexers{common.TagScopeSubnetCRUID: indexFunc})
	...
	service.InitializeResourceStore(&wg, fatalErrors, ResourceTypeSubnet, service.SubnetStore)
}
